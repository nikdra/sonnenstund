...
class _GeoSphereAustriaSunHoursParameter(_GeoSphereAustriaParameter):
    ...
    def sun_minutes(self):
        return [h * 60 for h in self.data] # for the future (other sources?) it might be good to standardize early
...
class _GeoSphereAustriaSunHoursQualityParameter(_GeoSphereAustriaParameter):
    ...
    @property
    def quality(self):
        return [self._quality_conversion(v) for v in self.data] # for the future (other sources?) it might be good to standardize early
